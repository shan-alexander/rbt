...
**Scope:** Abstract **data-engineering workflow** features for a medallion lake (bronze → silver → gold). Host product names, column vocabularies, and external orchestrators stay **out of core**; rbt *is* the transform/workflow engine for the lake.  
...
**Baseline version reviewed:** **0.7.3+** (contracts.enums / `--contract-diff` already on Unreleased)  
...
rbt is a **data-engineering workflow engine** with a sharp identity:

- **Medallion SQL DAG** on lake files (Parquet / parts / Iceberg-FS) — not a generic app framework.
- **Rust + DataFusion** for speed; materialize paths should stay stream-friendly and measurable.
- **Bronze → silver** is first-class: land mixed filetypes (jsonl, xml, txt, html, …) and publish **ultra-performant** stage models that turn those files into typed Parquet (line-as-row, file-as-column-value, path injection, hive partitions) so engineers write less glue than Spark/custom ETL.
- Prefer **lake-native** designs (Parquet/parts/Iceberg) over bolting on foreign stores. When a host wants “entity registry” speed, **optimize Parquet/parts/upsert-on-lake first**; only *consider* alternate stores after honest benches.

### Already shipped (do not re-implement)

| Capability | Where | Note |
|------------|--------|------|
| **Enum / value contracts** | `contracts.enums` in `rbt_project.yml` | Single registry for closed vocabularies (`works.source`, …) |
| **`rbt validate --contract-diff`** | `core/contracts.rs`, CLI validate | Sample bronze vs registry; `on_new: fail\|warn\|allow` |
| **Contract refs in tests** | `accepted_values: works.source` | Resolves at assert time via project config |

See [docs/COMPLEX_BRONZE_AND_RUN_SCOPE.md](../COMPLEX_BRONZE_AND_RUN_SCOPE.md) §7 and [docs/analysis/bronze-source-onboarding-friction.md](../analysis/bronze-source-onboarding-friction.md).  
**Do not** invent a second source registry or bronze “schema evolution” mini-language unless a new epic explicitly supersedes this.
...
9. **Stay on-brand:** lake files + DataFusion SQL + fast bronze adapters; durable external orchestration remains host-side.
...
> **Status:** **Implemented** on Unreleased (core `ScopeValue`, CLI `--var-file` / repeated `--var`,
> `require_partitions_in`, docs in `COMPLEX_BRONZE_AND_RUN_SCOPE.md`). Keep this section as the
> original brief; extend only if gaps appear.
...
## 10. Feature RBT-A8 — Storage backends research (**do not ship SQLite**)

### Decision (product)

**Do not implement SQLite (or other non-lake) storage as a product surface.**  
Shipping a second system of record splits rbt’s identity (lakehouse workflow on Parquet/Iceberg-FS + DataFusion) and pulls toward OLTP.

A8 is **research + measurement only**: if entity-registry hosts need point-update speed, beat or approach that with **on-brand** designs first.

### Goal (research)

1. **Cargo bench / measure** (R1–R5): full Parquet rewrite vs scoped parts vs future keyed_upsert-on-Parquet — honest numbers only.  
2. **Consider alternatives that stay lake-native**, e.g.:
   - `scoped_replace` / parts by entity key (A2 + A7)
   - Sorted Parquet + predicate pushdown / row-group pruning for point lookups
   - Small “hot” dimension as a single compact Parquet rewritten cheaply (N ≤ ~150k)
   - Iceberg-FS equality-delete / position-delete *only if* real multi-snapshot path exists (A11 honesty)
   - Optional **read-through cache** (in-process MemTable) for `ref()` of tiny dims — still publish Parquet
3. **SQLite** may appear only as a **bench baseline** (dev-dep / criterion harness), never as `storage.format: sqlite` in user frontmatter unless a future ADR overturns this decision with measured need.

### Explicit non-goals

- `rusqlite` product feature / Cargo feature for end users.  
- Dual-write Parquet + SQLite.  
- Documenting SQLite as the entity-registry path.

### Microtasks (research track)

| ID | Task | Detail |
|----|------|--------|
| **RBT-A8.1** | Bench harness only | Measure R1–R5 for Parquet full rewrite and (when A7 exists) keyed_upsert-Parquet. Optional: SQLite baseline **inside the bench crate only**. |
| **RBT-A8.2** | Design note | Short `docs/analysis/entity-registry-on-parquet.md`: when full rewrite is fine; when parts/scoped_replace win; prune strategies. |
| **RBT-A8.3** | A7 first | Implement Type-1 upsert **on Parquet/parts** before any foreign store discussion. |
| **RBT-A8.4** | ADR gate | Any future non-Parquet store requires ADR + benches beating lake-native options on stated workloads. **Default: reject.** |

### Acceptance

- No SQLite (or similar) in public `rbt` API / frontmatter.  
- Checked-in measure/bench numbers for lake-native entity-registry paths when A7 ships.
...
Backends to measure: **parquet full rewrite** · **keyed_upsert Parquet/parts** (A7).  
Optional: SQLite as **bench baseline only** (not product). Prefer proving lake-native is “good enough.”

**Do not** publish “sqlite is faster” without checked-in numbers — and do **not** ship SQLite as storage.
...
| Materialize | `materializer/incremental.rs`, new `materializer/scoped.rs`, new `materializer/upsert.rs` (Parquet/parts — **not** sqlite_store) |
